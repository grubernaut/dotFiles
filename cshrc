...
foreach d (/bin /usr/bin /sbin /usr/sbin /usr/X11R6 /usr/local/bin /usr/local/sbin /usr/games $GOPATH/bin $path)
	if ( -d $d ) then
...
Filesystem Input Events                 : %I\
Filesystem Output Events                : %O\
